...
class WelcomeViewController: NSViewController {
    ...
    private func setupStartButton() {
        let alert = NSAlert()
        alert.addButton(withTitle: "Get Started").keyEquivalent = "\r"
        
        guard let button = alert.buttons.first, let stackView = button.superview else {
            return
        }
        
        button.action = #selector(startButtonTapped)
        button.bezelColor = NSColor(srgbRed: 1, green: 0.27, blue: 1, alpha: 1)
        button.frame = CGRect(x: 0, y: 0, width: 212, height: 100)
        stackView.frame = CGRect(x: 214, y: -11, width: 260, height: 367)
        
        for subview in stackView.subviews where subview.tag != 1000{
        ...
        startButton.bezelColor = NSColor(displayP3Red: 206/255, green: 84/255, blue: 236/255, alpha: 1)
        startButton.bezelStyle = .regularSquare
        startButton.action = #selector(startButtonTapped)
        startButton.appearance = NSAppearance(named: .aqua)
    ...
    private func setupFeatures() {
        ...
        let feature1 = FeatureView(feature: syntax)
        ...
        let feature2 = FeatureView(feature: backgrounds)
        ...
        let feature3 = FeatureView(feature: export)
    ...
    private func initObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(becameActive), name: .init(rawValue: "NSApplicationDidBecomeActiveNotification"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(resignedActive), name: .init(rawValue: "NSApplicationDidResignActiveNotification"), object: nil)
        
        NSWorkspace.shared.notificationCenter.addObserver(self, selector: #selector(workspaceChanged), name: .init(rawValue: "NSWorkspaceActiveSpaceDidChangeNotification"), object: nil)
    }
    ...
    @objc private func workspaceChanged() {
        if NSWorkspace.shared.frontmostApplication?.bundleIdentifier != "com.sebvidal.Snippet" && windowFocused {
            DispatchQueue.main.async {
                self.startButton.appearance = NSAppearance(named: .aqua)
            }
        }
    }
}
